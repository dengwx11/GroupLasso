...
write.table(lamb_opt,"./lamb_opt_100", col.names = F, row.names = F,quote = F)
write.table(FDR_I,"./FDR_I_100", col.names = F, row.names = F,quote = F)
write.table(FDR_G,"./FDR_G_100", col.names = F, row.names = F,quote = F)
write.table(FNR_I,"./FNR_I_100", col.names = F, row.names = F,quote = F)
write.table(FNR_G,"./FNR_G_100", col.names = F, row.names = F,quote = F)
write.table(TP_I,"./TP_I_100", col.names = F, row.names = F,quote = F)
write.table(TP_G,"./TP_G_100", col.names = F, row.names = F,quote = F)
write.table(cnt_I,"./cnt_I_100", col.names = F, row.names = F,quote = F)
write.table(MSE_G,"./MSE_G_100", col.names = F, row.names = F,quote = F)
write.table(MSE_base,"./MSE_base_100", col.names = F, row.names = F,quote = F)
write.table(main_inter_ratio,"./main_inter_ratio_100",row.names = F,quote = F)

save(FDR_record_I,"./FDR_record_I_100.RData")
save(FDR_record_G,"./FDR_record_G_100.RData")
save(FNR_record_I,"./FNR_record_I_100.RData")
save(FNR_record_G,"./FNR_record_G_100.RData")
save(TDR_record_I,"./TDR_record_I_100.RData")
save(TDR_record_G,"./TDR_record_G_100.RData")
save(TNR_record_I,"./TNR_record_I_100.RData")
save(TNR_record_G,"./TNR_record_G_100.RData")
save(compare_sign_G_total, file="./compare_sign_G_100.RData")
save(compare_sign_I_total, file="./compare_sign_I_100.RData")
...
library(ggplot2)
library(ggridges)
FDR_G_wide<-data.frame(FDR_G)
FDR_G_wide$id<-c(1:100)
FDR_G_long<-reshape(FDR_G_wide,idvar = "id",direction = "long", varying = list(1:3))
ggplot(FDR_G_long,aes(X1,fill=factor(time)))+ geom_histogram()
ggplot(FDR_G_long, aes(x=X1 ,y = factor(time))) +geom_density_ridges(quantile_lines=T)

FDR_I_wide<-data.frame(FDR_I)
FDR_I_wide$id<-c(1:100)
FDR_I_long<-reshape(FDR_I_wide,idvar = "id",direction = "long", varying = list(1:3))
ggplot(FDR_I_long,aes(X1,fill=factor(time)))+ geom_histogram()
ggplot(FDR_I_long, aes(x=X1 ,y = factor(time))) +geom_density_ridges(quantile_lines=T)

FNR_G_wide<-data.frame(FNR_G)
FNR_G_wide$id<-c(1:100)
FNR_G_long<-reshape(FNR_G_wide,idvar = "id",direction = "long", varying = list(1:3))
ggplot(FNR_G_long,aes(X1,fill=factor(time)))+ geom_histogram()
ggplot(FNR_G_long, aes(x=X1 ,y = factor(time))) +geom_density_ridges(quantile_lines=T)

FNR_I_wide<-data.frame(FNR_I)
FNR_I_wide$id<-c(1:100)
FNR_I_long<-reshape(FNR_I_wide,idvar = "id",direction = "long", varying = list(1:3))
ggplot(FNR_I_long,aes(X1,fill=factor(time)))+ geom_histogram()
ggplot(FNR_I_long, aes(x=X1 ,y = factor(time))) +geom_density_ridges(quantile_lines=T)

MSE_G_wide<-data.frame(MSE_G)
MSE_G_wide$id<-c(1:100)
MSE_G_long<-reshape(MSE_G_wide,idvar = "id",direction = "long", varying = list(1:3))
ggplot(MSE_G_long,aes(X1,fill=factor(time)))+ geom_histogram()
ggplot(MSE_G_long, aes(x=X1 ,y = factor(time))) +geom_density_ridges(quantile_lines=T)

MSE_base_wide<-data.frame(MSE_base)
MSE_base_wide$id<-c(1:100)
MSE_base_long<-reshape(MSE_base_wide,idvar = "id",direction = "long", varying = list(1:3))
ggplot(MSE_base_long,aes(X1,fill=factor(time)))+ geom_histogram()
ggplot(MSE_base_long, aes(x=X1 ,y = factor(time))) +geom_density_ridges(quantile_lines=T)

lamb_opt_wide<-data.frame(lamb_opt)
lamb_opt_wide$id<-c(1:100)
lamb_opt_long<-reshape(lamb_opt_wide,idvar = "id",direction = "long", varying = list(1:3))
ggplot(lamb_opt_long,aes(X1,fill=factor(time)))+ geom_histogram()
ggplot(lamb_opt_long, aes(x=X1 ,y = factor(time))) +geom_density_ridges(quantile_lines=T)
...
lamb_opt<-.5
sol<-FASTA2(X,y,f, gradf, g2, proxg2, x0, tau1, max_iters = 500, w = 10, 
            backtrack = TRUE, recordIterates = FALSE, stepsizeShrink = 0.5, 
