...
for(i in 1:t){
  ...
  true_beta<-sim_beta(m_X,m_W,p_G,main_zero,inter_zero)

  
  X<-sim_X(m_X,m_W,p_G)
  y<-X%*%true_beta+rnorm(n)
  
  true_beta<-split_beta(true_beta,m_X,m_W,m_G1,m_G2,m_I)
  true_beta$G<-true_beta$G1+true_beta$G2
  ...
  sol<-FASTA(X,y,f, gradf, g, proxg, x0, tau1, max_iters = 500, w = 10, 
             backtrack = TRUE, recordIterates = FALSE, stepsizeShrink = 0.5, 
             eps_n = 1e-15,m_X,m_W,m_G1,m_G2,m_I,lamb_opt,lamb_opt,restart=TRUE)
  estbeta<-split_beta(sol$x,m_X,m_W,m_G1,m_G2,m_I)
