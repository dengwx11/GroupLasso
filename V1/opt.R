...
FASTA<-function (X,y,f, gradf, g, proxg, x0, tau1, max_iters = 100, w = 10, 
          backtrack = TRUE, recordIterates = FALSE, stepsizeShrink = 0.5, 
          eps_n = 1e-15,m_X,m_W,m_G1,m_G2,m_I,lambda1,lambda2,restart=TRUE) 
{
  ...
  objective[1] <- f1 + g(X,x0,m_X,m_W,m_G1,m_G2,m_I,lambda1,lambda2)
  t0=1
  y0<-x0
  for (i in 1:max_iters) {
    ...
    x1 <- proxg(X,x1hat,m_X,m_W,m_G1,m_G2,m_I, tau0, lambda1,lambda2)
    Dx <- matrix(x1 - x0)
    d1 <- x1
    f1 <- f(d1,X,y)
    if (backtrack) {
    ...
    fVals[i] <- f1
    objective[i + 1] <- f1 + g(X,x1,m_X,m_W,m_G1,m_G2,m_I,lambda1,lambda2)
